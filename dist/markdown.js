...
exports.splitIgnoringQuotes = splitIgnoringQuotes;
...
class MarkdownNode {
    ...
    async evaluate(currentUniverse, ctx, transform, topLevel = true) {
        if (this.type === 'a') {
            this.addClass('link');
...
class CtxLookup {
    ...
    getValue(ctx) {
        ...
        for (const key of this.lookup) {
            if (value && (key in value || (value instanceof Array && Number(key) < value.length)))
                value = value[key];
            else {
...
function parseInline(line) {
    ...
    while (line.hasNext()) {
        ...
        if (char === '\\') {
            ...
            const boldNode = new MarkdownNode(stars === 0 ? 'b' : 'text', stars === 0 ? '' : escaped);
            nodes.push(boldNode);
            ...
            const italicsNode = new MarkdownNode(stars === 0 ? 'i' : 'text', stars === 0 ? '' : char);
            ...
            if (line.next() === ']' && line.next() === '(') {
                ...
                if (line.next() === ')') {
                    ...
                    nodes.push(linkNode);
                    if (isImage)
                        linkNode.attrs.alt = lineNodes.map(node => node.innerText()).join('');
                    else
    ...
    for (const line of lines) {
        ...
        if (line[0] === '#') {
            ...
            if (line[j] === ' ') {
                ...
                if (!toc)
                    continue;
                if (maxTocDepth && headingLvl > maxTocDepth)
                    continue;
                ...
                if (headingLvl > lastHeadingLvl) {
                    ...
                    for (let i = 0; i < lastHeadingLvl - headingLvl; i++) {
                        if (newListNode.parent?.parent?.type === 'ol')
                            newListNode = newListNode.parent.parent;
                        else
                            break;
                    }
                    curTocList = [newListNode, headingLvl];
                }
                const [curListNode] = curTocList; // We know a node has been set
            ...
            if (indent > lastIndent) {
                ...
                if (!lastListItem)
                    rootList = newListNode;
                curList = [newListNode, indent];
            }
            else if (indent < lastIndent) {
                let newListNode = lastListNode;
                for (let i = 0; i < lastIndent - indent; i++)
                    newListNode = newListNode.parent?.parent;
                curList = [newListNode, indent];
            }
            const [curListNode] = curList; // We know a node has been set
            curListNode.addChild(new MarkdownNode('li'));
            curListNode.lastChild()?.addChildren(parseInline(new Line(trimmedLine.substring(2))));
            rootList?.addSrc(line);
        }
        else if (trimmedLine === '') {
            if (curParagraph.hasChildren())
                root.addChild(curParagraph);
            curParagraph = new MarkdownNode('p');
            curList = [null, -1];
        }
