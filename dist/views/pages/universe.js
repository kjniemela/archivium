...
module.exports = {
    async list(req, res) {
        ...
        const [code, universes] = await api.universe.getMany(req.session.user, search ? { strings: ['title LIKE ?'], values: [`%${search}%`] } : null, perms.READ, {
            sort: req.query.sort,
    ...
    async view(req, res) {
        ...
        const couldUpgrade = sponsored ? (sponsored.length === 0 || sponsored
            ...
            .some(row => row.universes.length < tierAllowance[user.plan][row.tier])) : false;
    ...
    async createDiscussionThread(req, res) {
        ...
        if (!universe.discussion_open && universe.author_permissions[req.session.user.id] < perms.COMMENT)
            return res.status(403);
        res.status(code);
        if (code !== 200)
            return;
    ...
    async editPerms(req, res) {
        ...
        for (const userID in universe.author_permissions) {
            if (universe.author_permissions[userID] === perms.OWNER)
                ownerCount++;
        }
