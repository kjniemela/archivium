...
async function getByUsername(sessionUser, username, conditions, options) {
    try {
        ...
        if (!user)
            return [code];
        const [_, notes] = await getMany(sessionUser, { ...(conditions ?? {}), 'note.author_id': user.id }, options ?? {});
        return [200, notes];
    }
    catch (err) {
        logger.error(err);
...
async function getByItemShortname(user, universeShortname, itemShortname, conditions, options, inclAuthors = false) {
    try {
        ...
        if (!item)
            return [code];
        const [_, notes] = await getMany(user, { ...conditions ?? {}, 'itemnote.item_id': item?.id }, { ...options ?? {} });
        if (inclAuthors) {
            const queryString2 = `
...
async function getByBoardShortname(user, shortname, conditions, options, validate = true, inclAuthors = false) {
    try {
        ...
        const [_, notes] = await getMany(user, { ...conditions ?? {}, 'boardnote.board_id': board.id }, { ...options ?? {} });
        if (inclAuthors) {
            const queryString2 = `
        SELECT user.id, user.username, user.email
        FROM user
