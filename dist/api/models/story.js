...
async function post(user, payload) {
    ...
    if (!shortname)
        return [400, 'Shortname is required.'];
    if (!universeShort)
        return [400, 'Universe is required.'];
    if (typeof public !== 'boolean')
        return [400, 'Draft visibility status is required.'];
...
async function delChapter(user, shortname, index) {
    ...
    try {
      ...
      `, [chapter.id]);
        ...
        const [code, story] = await getOne(user, { 'story.shortname': shortname }, perms.OWNER);
        if (!story)
            throw new Error(`Fatal errror: story.getOne returned code ${code} after chapter deletion!`);
