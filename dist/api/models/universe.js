...
async function getMany(user, conditions, permissionLevel = perms.READ, options = {}) {
    ...
    try {
        if (!user && permissionLevel > perms.READ)
            return [400];
...
async function post(user, body) {
    ...
    `;
        ...
        if (err.code === 'ER_DUP_ENTRY')
            return [400, 'Universe shortname must be unique.'];
...
async function putPermissions(user, shortname, targetUser, permission_level) {
    ...
    const [code, universe] = await getOne(user, { shortname }, permission_level === perms.OWNER ? perms.OWNER : Math.max(perms.ADMIN, permission_level + 1));
    if (!universe)
        return [code];
    ...
    try {
        await executeQuery('DELETE FROM universeaccessrequest WHERE universe_id = ? AND user_id = ?', [universe.id, targetUser.id]);
...
async function putUserSponsoring(user, shortname, tier) {
    ...
    if (tier === tiers.FREE) {
        ...
        if (sponsoredAtTier >= tierAllowance[user.plan][tier] ?? 0)
            return [402];
        if (universe.tier === null) {
            query = executeQuery(`
...
async function getUserAccessRequest(user, shortname) {
    ...
    try {
        ...
        if (!universe)
            return [404];
        const request = (await executeQuery('SELECT * FROM universeaccessrequest WHERE universe_id = ? AND user_id = ?', [universe.id, user.id]))[0];
        if (!request)
            return [404];
        return [200, request];
    }
...
async function getAccessRequests(user, shortname) {
    ...
    try {
        ...
        if (!universe)
            return [code];
        const requests = await executeQuery('SELECT ua.*, user.username FROM universeaccessrequest ua INNER JOIN user ON user.id = ua.user_id WHERE ua.universe_id = ?', [universe.id]);
        return [200, requests];
    }
    catch (err) {
        logger.error(err);
    ...
    try {
        ...
        if (request) {
            if (request.permission_level >= permissionLevel)
                ...
                await delAccessRequest(user, shortname, user);
        }
        const data = await executeQuery('INSERT INTO universeaccessrequest (universe_id, user_id, permission_level) VALUES (?, ?, ?)', [universe.id, user.id, permissionLevel]);
        return [201, data];
    }
    catch (err) {
        logger.error(err);
...
async function delAccessRequest(user, shortname, requestingUser) {
    ...
    if (!(permsUniverse || (code === 403 && user.id === requestingUser.id)))
        return [code];
    try {
        const universe = (await executeQuery('SELECT * FROM universe WHERE shortname = ?', [shortname]))[0];
        await executeQuery('DELETE FROM universeaccessrequest WHERE universe_id = ? AND user_id = ?', [universe.id, requestingUser.id]);
        return [200];
    }
    catch (err) {
        logger.error(err);
