...
class UserModel extends _1.Model {
    ...
    static async findAll(conds = {}) {
        ...
        const rows = await _1.Model.query(`SELECT * FROM ${UserModel.TABLE} WHERE ${str}`, values);
    ...
    async save() {
        if (this.id === null) {
            ...
            const { insertId } = await _1.Model.insert(UserModel.TABLE, this.toJSON());
            this.id = insertId;
        }
        else {
            await _1.Model.update(UserModel.TABLE, this.id, this.toJSON());
        }
    }
    async delete() {
        if (this.id !== null) {
            await _1.Model.query(`DELETE FROM ${UserModel.TABLE} WHERE id = ?`, [this.id]);
        }
    }
    toJSON() {
        return {
