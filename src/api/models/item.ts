...
class ItemImageAPI {
  ...
  async putLabel(user: User, imageId: number, label: string, conn?: PoolConnection): Promise<ResultSetHeader> {
    if (!user) throw new UnauthorizedError();
  ...
  async del(user: User, imageId: number, conn?: PoolConnection): Promise<void> {
    if (!user) throw new UnauthorizedError();
    const images = await this.getMany({ id: imageId }, false) as ItemImage[];
    const image = images && images[0];
    if (!image) throw new NotFoundError();
...
export class ItemAPI {
  ...
  async save(user: User, universeShortname: string, itemShortname: string, body: Partial<Item>): Promise<number> {
    let item!: Item;
    await withTransaction(async (conn) => {
      // Actually save item
      const changes = {
  ...
  async putTags(user: User, universeShortname: string, itemShortname: string, tags: string[], conn?: PoolConnection): Promise<ResultSetHeader | void> {
  ...
  async delTags(user: User, universeShortname: string, itemShortname: string, tags: string[], conn?: PoolConnection): Promise<ResultSetHeader | void> {
    if (tags.length === 0) return; // Nothing to do
    const item = await this.getByUniverseAndItemShortnames(user, universeShortname, itemShortname, perms.WRITE, true);
